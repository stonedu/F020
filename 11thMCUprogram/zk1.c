...
//ADDR Setting
#define A0  (P5&0x01)  //P5¿ÚÎª²¦Âë¿ª¹Ø
#define A1  (P5&0x02)
#define A2  (P5&0x04)
#define A3  (P5&0x08)
#define baud_mode (P5&0xf0)  //¹¤×÷Ä£Ê½

#define ENQ 0x05  //Ñ¯ÎÊ
#define ACK 0x06  //È·ÈÏ
#define NAK 0x15  //·ñÈÏ
#define EOT 0x04  //·¢ËÍ½áÊø
#define ETX 0x03  //Ó¦´ğ½áÊø
#define DB  0x01  //×Ö½Ú
#define DW  0x02  //×Ö
#define DF  0x03  //¸¡µã
#define BufMax 30 //¶ÁÊı¾İ¸öÊı
#define time0  -100000

#define P1_ADDR 	0  //P1¿Ú×éÌ¬ÍõµÄµØÖ·
#define P2_ADDR 	2  //P2¿Ú¶ÔÓ¦µÄ×éÌ¬ÍõµÄµØÖ·
#define DA0_ADDR 	20 //DA0¶ÔÓ¦µÄ×éÌ¬ÍõµÄµØÖ·
#define DA1_ADDR 	22
#define AD_ADDR 	4  //AD¿Ú¶ÔÓ¦µÄ×éÌ¬ÍõÖĞµÄµØÖ·
#define LIGHT_ADDR 	24 //DI¿Ú¶ÔÓ¦µÄ×éÌ¬ÍõÖĞµÄµØÖ·
#define Start 0x40
#define End 0x0d
#define Read 0x00
#define Write 0x01
...
uchar flag=0;    //¶à»úÊ¶±ğ±êÖ¾Î»


uchar TH,TL;

uchar rd_data[8];     //Êı×érd_data[8]ÖĞ´æ·Å´ÓDS18B20ÖĞ¶Á³öµÄÊı¾İ
//................//
...
//18B20 Ö÷»ú·¢Ìø¹ı¶ÁĞòÁĞºÅµÄ²Ù×÷ÃüÁî:0xcc
...
uint erheyi(uchar high,uchar low);///////¸ßÎ»ºÍµÍÎ»ºÏ³É
...
void sleep_ms(unsigned int count);      //ÑÓÊ±
...
//	uint zhuan;
	...
	if(baud_mode==0x80) //0001 VB
	{
		...
		Init18b20();//³õÊ¼»¯18b20
    	w = GetTempValue();            //¶Á³öÎÂ¶ÈÖµµÄÔ­Ê¼Êı¾İ´æÈëW
	}
	else if(baud_mode==0x00) //0000 ×éÌ¬Íõhex
	{
		EA=1;
		ES0=1; 
		SCON0 = 0xD0; //ÓĞĞ£Ñé
		Init18b20();//³õÊ¼»¯18b20
		WorkMode = 0;//9600,×éÌ¬Íõ
	}
	else if(baud_mode==0x40)   //0010 ×éÌ¬ÍõASC
	{	
		EA=1;
		ES0=1;
		SCON0=0xD0;//ÓĞĞ£Ñé
	...
	else if(baud_mode==0xc0) //0011 MATLAB
	{
		...
		//////////////////////////////
		TMOD=0x10;//¶¨Ê±Æ÷1 16Î»    4ms
		TH1=0xf0;//9600²¨ÌØÂÊÏÂ3.5¸ö×Ö½ÚµÄÊ±¼ä´óÔ¼Îª3.6MS£¬ÎÒÔÚÕâÀïÉèÖÃÎª4ms
		TL1=0x5f;
	//	TL0=TH0=0xFF;
		ET1=1;
	//	TR1=1;
	...
	else if(baud_mode==0xa0) //0101 ÑĞ»ªmodbus
	{
		EA=1;
		ES0=1;
		SCON0=0x50;//ÎŞĞ£Ñé
	...
	else if(baud_mode==0xf0) //1111 VBÀÏ×Ô¿Ø
	{
		EA=0;
		ES0=0; 
		SCON0 = 0x50;   //ÎŞĞ£Ñé
...
//    Init_Device();	     //initialization device
	DAC0L=0x00;
	DAC0H=0x08;          //³õÊ¼»¯daÊä³öÖµ
	DAC1L=0x00;
	DAC1H=0x08;          //³õÊ¼»¯daÊä³öÖµ
	P6 = 0xFF;
	//SCON0 = SCON0 | 0x20;
	...
	/***************************   VB   *************************************/
	while(WorkMode==1)  //VBÄ£Ê½
	{
		while(RI0==0) {}
		RI0=0;
		tmpAddr=SBUF0; //½ÓÊÕÃüÁîµØÖ·
		if(tmpAddr == MyAddr)
		{
		     while(RI0==0){}
			 RI0=0;
			 tmpCommand=SBUF0; //½ÓÊÕÃüÁî×Ö
			 if(0<=tmpCommand)
			 {
			     while(RI0==0){}
				 RI0=0;
				 ...
				 else if(tmpCommand==1)  //da×ª»» 1Í¨µÀ
				 {
				     DAC1L=value2;		//µÍÎ»
					 DAC1H=value1;		//¸ßÎ»
					 UART_Send(0x55);
				 ...
				 else if(tmpCommand==4)	 //DI P1¿ÚÊı×ÖÁ¿ÊäÈë
				 {
				 	P1_Status = P1;
                    UART_Send(0);
					UART_Send(P1_Status);
				 }
				 else if(tmpCommand==5)	 //DI P2¿ÚÊı×ÖÁ¿ÊäÈë
				 {
				 	P2_Status = P2;
                    UART_Send(0);
					UART_Send(P2_Status);
				 }
				 else if(tmpCommand==6)	 //DI P6¿ÚÊı×ÖÁ¿ÊäÈë
				 {
				 	P6_Status = P6;
                    UART_Send(0);
					UART_Send(P6_Status);
				 }
				 else if(tmpCommand==0x30)  //¶ÁÈ¡18B20µÄÊı¾İ
				 {
				 	w = GetTempValue();            //¶Á³öÎÂ¶ÈÖµµÄÔ­Ê¼Êı¾İ´æÈëW
					UART_Send(TH);
					UART_Send(TL);
				 }
				 else if(tmpCommand==0x40)  //TFTÏÔÊ¾Ë¢ÆÁ
				 {
				 	UART_Send(0x55);
					UART_Send(0x55);
				 }
				 else if(tmpCommand==0xFF)	 //½Úµã²âÊÔ
				 {
	...
	while(WorkMode==0) //×éÌ¬ÍõÄ£Ê½  hex
	{
		...
		//	SCON0 = SCON0 | 0x20;
			CheckCRC();
 	   		if(Rbuf[0]==MyAddr && CRCok==1)  //¶ÔPCµÄ²éÑ¯ÃüÁî½øĞĞÓ¦´ğ
	   		{
	   		...
	   		else if(CRCok==1 && flag==1)     //ÔÚÓ¦´ğºó½øĞĞÊı¾İ´«ËÍ¿ØÖÆ
	   		{	  							//µ±ÊÇ¶Á»òĞ´Ö¡Ê±£¬¾ÍÒªÅĞ¶ÏÒ»ÏÂÉÏÒ»Ö¡ÖĞµÄÉè±¸µØÖ·ÊÇ·ñÊÇ×Ô¼ºµÄÉè±¸£¬Èç¹ûÊÇÔò·¢Êı¾İ£¬·ñÔò²»·¢¡£	
				flag=0;
          		ReadMe=2;
		      	n=Rbuf[4];
			  	ptr=Rbuf[3]<<8 | Rbuf[2];  
		      	if(Rbuf[0]=='R')
	      		{
		     		switch(Rbuf[1])
		     		{
			   			...
			   			case DW:num=n/2; 	
			           		for(i=0;i<num;i++)
					   		{
					     		k=i*4;   //6.55ºÍ6.53µÄÇø±ğ
								ADC_Start(i);
					     		Tbuf[3+k]=ADC0L;
					     		Tbuf[4+k]=ADC0H;
					   		}
							if(Rbuf[2]==30)//Èç¹ûÊÇÎÂ¶ÈÊı¾İÇëÇó£¬ÔòÖ´ĞĞÕâÌõÓï¾ä
                            {
				            	w = GetTempValue();            //¶Á³öÎÂ¶ÈÖµµÄÔ­Ê¼Êı¾İ´æÈëW
								//Tbuf[3]=0x6F;
								//Tbuf[4]=0xFE;
		  		...
		  		{
             		n=Rptr-6;
		     		switch(Rbuf[1])//ÅĞ¶ÏÒªĞ´µÄÊı¾İÀàĞÍ
		     		{
			   			case DB: 
						    //if(Rbuf[2]==1)
			           		{P6 = (~Rbuf[4]);}
	...
	//			SM2=0;
			...
			if(temp==MyAddr)//&&check_CRC()
				...
				if(read_write==Read) 
				{
					n=Dat_trans(recbuf[9],recbuf[10]);//È¡×Ö½ÚÊı
					x=read_write_flag()&0x0c;    //¶ÁĞ´Êı¾İÀàĞÍ×Ö½Ú»ò×Ö·
					switch(x)
					{
						case 0x00:num=n;
							...
							if(Dat_trans(recbuf[7],recbuf[8])==0)
							{
								...
								break;
						case 0x04:num=n/2;   //×Ö   ÏÂÃæADC0LÓëADC0H¸÷Ò»¸ö×Ö½Ú¾ÍÂú×ãÁË
							//	n=2*n;
								for(i=0;i<num;i++)
								{
								  dizhir=Dat_trans(recbuf[7],recbuf[8]);//µØÖ·
									if(dizhir==0x02) {ko=0;}
									if(dizhir==0x04) {ko=1;}
									if(dizhir==0x06) {ko=2;}
									if(dizhir==0x08) {ko=3;}
									...
									ADC_Start(ko);
									send_temp1=ADC0H;
					     			send_temp2=ADC0L;//asc¸ßÎ»ÔÚÇ°µÍÎ»ÔÚºó£¬HEXµÍÎ»ÔÚÇ°£¬¸ßÎ»ÔÚºó
								...
								if(Dat_trans(recbuf[7],recbuf[8])==0x1e)//Èç¹ûÊÇÎÂ¶ÈÊı¾İÇëÇó£¬ÔòÖ´ĞĞÕâÌõÓï¾ä,30µÄÊ®Áù½øÖÆÎª0x1e
                            	{
								for(i=0;i<num;i++)
								{
									k=i*4;
									...
									//Tbuf[4]=0xFE;
									send_temp1=TH;
					     			send_temp2=TL;//asc¸ßÎ»ÔÚÇ°µÍÎ»ÔÚºó£¬HEXµÍÎ»ÔÚÇ°£¬¸ßÎ»ÔÚºó
									sendbuf[5+k]=HEXASC_high(send_temp1);
									sendbuf[6+k]=HEXASC_low(send_temp1);
									sendbuf[7+k]=HEXASC_high(send_temp2);
									sendbuf[8+k]=HEXASC_low(send_temp2);
				...
				switch(x)
					...
					case 0x00:num=n;         //×Ö½Ú
						...
						P6 = (~tempw);
			      		break;
					case 0x04:num=n/2;									//×Ö£¬Ã»ÓĞ·ÖÎ»£¬¾ÍÊÇÁ½¸ö×Ö½Ú
						dizhiw=Dat_trans(recbuf[7],recbuf[8]);
						tempw1=Dat_trans(recbuf[11],recbuf[12]);
						tempw2=Dat_trans(recbuf[13],recbuf[14]);
							if(dizhiw==0x14) //DA0_ADDRÒòÎª==20²»ÄÜ¼ì²â³öÀ´£¬20µÄÊ®Áù½øÖÆÎªËùÒÔ¸ÄÎª0x14£¬ÆäÊµÊÇDA0_ADDR
							{
								
								DAC0H=tempw1;//asc¸ßÎ»ÔÚÇ°µÍÎ»ÔÚºó£¬HEXµÍÎ»ÔÚÇ°£¬¸ßÎ»ÔÚºó
								DAC0L=tempw2;
								DAC0H=tempw1;//51bug,±ØĞë¸øDA¸³ÖµÁ½´Î£¬Ò»´Î²»¾«×¼
								DAC0L=tempw2;//tempw2;
												
							}
							else if(dizhiw==0x16) //DA1_ADDRÍ¬ÉÏ
			        		{
								DAC1H=tempw1;//tempw1;
								DAC1L=tempw2;//tempw2;
								DAC1H=tempw1;//51bug,±ØĞë¸øDA¸³ÖµÁ½´Î£¬Ò»´Î²»¾«×¼
								DAC1L=tempw2;//tempw2;
...
//				 SM2=1;
	...
	//			SM2=0;
			...
			if(temp==MyAddr)//&&check_CRC()
				{	
					...
					read_write=Dat_trans(matRbuf[3],matRbuf[4]);  //¶ÁĞ´±êÖ¾Î»
				if(read_write==Read)   //00
				{
					n=Dat_trans(matRbuf[9],matRbuf[10]);//È¡×Ö½ÚÊı
					x=Dat_trans(matRbuf[5],matRbuf[6]);    //¶ÁĞ´Êı¾İÀàĞÍ×Ö½Ú»ò×Ö·
					switch(x)
					{
						case 0x00:num=n;
							...
							if(Dat_trans(matRbuf[7],matRbuf[8])==0)
							{
								...
								break;
						case 0x01:num=n/2;   //×Ö   ÏÂÃæADC0LÓëADC0H¸÷Ò»¸ö×Ö½Ú¾ÍÂú×ãÁË
							//	n=2*n;
								for(i=0;i<num;i++)
								{
								  dizhir=Dat_trans(matRbuf[7],matRbuf[8]);//µØÖ·
									if(dizhir==0x02) {ko=0;}
									if(dizhir==0x04) {ko=1;}
									if(dizhir==0x06) {ko=2;}
									if(dizhir==0x08) {ko=3;}
								...
								if(Dat_trans(matRbuf[7],matRbuf[8])==0x1e)//Èç¹ûÊÇÎÂ¶ÈÊı¾İÇëÇó£¬ÔòÖ´ĞĞÕâÌõÓï¾ä,30µÄÊ®Áù½øÖÆÎª0x1e
                            	{
								for(i=0;i<num;i++)
								{
									k=i*4;
				...
				switch(x)
					...
					case 0x00:num=n;         //×Ö½Ú
						...
						P6 = (~tempw);
			      		break;
					case 0x01:num=n/2;									//×Ö£¬Ã»ÓĞ·ÖÎ»£¬¾ÍÊÇÁ½¸ö×Ö½Ú
		...
		while(WorkMode==4)
		{
			...
			{	
				if(labRbuf[1]==MyAddr) //dizhi
				{
									  ...
									  for(i=0;i<num;i++)
										{
											...
											ADC_Start(ko);
											labTbuf[3]=ADC0H;
					     					labTbuf[4]=ADC0L;//asc¸ßÎ»ÔÚÇ°µÍÎ»ÔÚºó£¬HEXµÍÎ»ÔÚÇ°£¬¸ßÎ»ÔÚºó
											
										}
									 	if(labRbuf[4]==0x1e)//Èç¹ûÊÇÎÂ¶ÈÊı¾İÇëÇó£¬ÔòÖ´ĞĞÕâÌõÓï¾ä,30µÄÊ®Áù½øÖÆÎª0x1e
                            			{
							 			 for(i=0;i<num;i++)
										{
											k=i*4;
					     				...
					     				//	labTbuf[4]=0X6f;
											labTbuf[3]=TH;
					     					labTbuf[4]=TL;//asc¸ßÎ»ÔÚÇ°µÍÎ»ÔÚºó£¬HEXµÍÎ»ÔÚÇ°£¬¸ßÎ»ÔÚºó
					...
					{
		     			switch(labRbuf[3])//ÅĞ¶ÏÒªĞ´µÄÊı¾İÀàĞÍ
		     			{
			   				case 0x00: 
						   		 //if(labRbuf[4]==0x00)
			           			{P6 = (~labRbuf[6]);}
				...
				if(modRbuf[0]==MyAddr && modRbuf[1]==0x03) //dizhi 
				{
					...
					if(crc16(modRbuf,6)==crcr)
					{
						...
						if(modRbuf[3]+modRbuf[5]>8 || modRbuf[2]>0 || modRbuf[4]>0)
						{
							...
							modTbuf[2]=0;
							crct=crc16(modTbuf,3);//¸öÊıÒª¸Ä
							modTbuf[3]=crct&0x00ff;//bufÎ»ÖÃ¸Ä
							modTbuf[4]=crct>>8;	
						...
						else
						{
							...
							crct=crc16(modTbuf,3+modRbuf[5]*2);//¸öÊıÒª¸Ä
							modTbuf[3+modRbuf[5]*2]=crct&0x00ff;//bufÎ»ÖÃ¸Ä
							modTbuf[4+modRbuf[5]*2]=crct>>8;
							UART0_SendBuf(modTbuf,5+modRbuf[5]*2);//·¢ËÍ¸öÊıÒª¸Ä
						}
					}//crc
				}//addr
				modrecok=0;
	...
	/************************Matlab Simulink*******************************/
	while(WorkMode==7)  //Matlab SimulinkÄ£Ê½
	{
		while(RI0==0) {}
		RI0=0;
		tmpAddr=SBUF0; //½ÓÊÕÃüÁîµØÖ·
		if(tmpAddr == MyAddr)
		{
			 ...
			 if(0<=tmpCommand)
			 {
				 ...
				 else if(tmpCommand==1)  //da×ª»» 1Í¨µÀ
				 {
					 ...
					 UART_Send(0x55);
					 UART_Send(0x55);
					 UART_Send(0x55);   //·¢ËÍ4´Î0x55
				 }
				 else if(tmpCommand==2)	 //ad×ª»»		 
				 {
				     ADC_Start(value2);
					 UART_Send(tmpAddr);  //·¢ËÍµØÖ·
					 UART_Send(value2);   //·¢ËÍÍ¨µÀºÅ
					 UART_Send(ADC0H);
					 UART_Send(ADC0L);     //·¢ËÍAD×ª»»Êı¾İ
				 }				 
			 }
		 }
		 else
			...
			if(modrecok==1)
			{
				if(modRbuf[0]==MyAddr && modRbuf[1]==0x03) //dizhi+AD 
				{
					...
					if(crc16(modRbuf,6)==crcr)
					{
						...
						if(modRbuf[3]+modRbuf[5]>8 || modRbuf[2]>0 || modRbuf[4]>0) //ÊäÈë´íÎóÅĞ¶Ï
						{
							modTbuf[0]=modRbuf[0];
							modTbuf[1]=modRbuf[1]|0x80;
							modTbuf[2]=0;
				...
				else if(modRbuf[0]==MyAddr && modRbuf[1]==0x06)//dizhi+DA+DO
				{
					...
					if(crc16(modRbuf,6)==crcr)
					{
						...
						{	
							if(modRbuf[3]==0x08)  //DA0×ª»»
							{
				     			DAC0L=modRbuf[5];		//µÍÎ»
					 			DAC0H=modRbuf[4];      //¸ßÎ»
							}
							if(modRbuf[3]==0x09)  //DA1×ª»»
							{
				     			DAC1L=modRbuf[5];		//µÍÎ»
					 			DAC1H=modRbuf[4];      //¸ßÎ»
							}
							if(modRbuf[3]==0x0a)  //DA0×ª»»
							{
				     			P6=~modRbuf[5];		//µÍÎ»
							}
...
void ADC_Start(uchar source)
{
    AMX0SL = source;	  //Ñ¡ÔñÍ¨µÀ
	AD0INT = 0;
	AD0BUSY = 1;          //Æô¶¯AD×ª»»
	while (!AD0INT){}	  //µÈ´ı×ª»»Íê³É
	AD0INT =0;
							//Á½´Î×ª»»
	AD0BUSY = 1;          //Æô¶¯AD×ª»»
	while (!AD0INT){}	  //µÈ´ı×ª»»Íê³É
...
bit RstDS1820(void)		//·µ»Ø0-ÓĞÉè±¸Á¬½Ó1-ÎŞÉè±¸Á¬½Ó
{
	unsigned char i;
	bit RstFlag;
	RstFlag=1;	
	DataPort_Set0;
	for (i=0;i<40;i++)      	//480us ÑÓÊ±
		Delay15us(); 
	DataPort_Set1;
	for(i=0;i<4;i++)			//15us-60us ÑÓÊ±
	{
		Delay15us();
	}
	for (i=0;i<16;i++)			//60us-240us ÑÓÊ±
	{
		Delay15us();
...
void Delay1us(unsigned char us)  //ÑÓÊ±
{
	while (us)
	{
...
void SkipROMCode(void)       //Ö÷»ú·¢Ìø¹ı¶ÁĞòÁĞºÅµÄ²Ù×÷ÃüÁî:0xcc
{
	RstDS1820();
	WriteDS1820(0xcc);
}
...
void StartDS(void)
{
	if(RstDS1820()) UART_Send(0x80);;			//¸´Î»
	WriteDS1820(0xcc);		//¹ã²¥
	WriteDS1820(0x44);		//Æô¶¯ÎÂ¶È×ª»»×ª»» 12bit700ms
}
unsigned int GetTempValue(void)//¶ÁÈ¡ÎÂ¶ÈÖµµÄ³ÌĞò
{
	...
	sleep_ms(250);
	SkipROMCode();
	WriteDS1820(0xBE);  //·¢²¼¶ÁÈ¡ÎÂ¶ÈÖµµÄÃüÁî(0xBE)
	...
	if (T>0x8000)                      //ÎÂ¶ÈÎª¸ººÅ
	{
		T=-T;
		return(0x8000+T*5/8);          //¼ÆËãÎÂ¶ÈÖµ
	}
	else                               //ÎÂ¶ÈÎªÕıºÅ
		return(T*5/8);
		*/
...
void Init18b20(void)
{
	SkipROMCode();                        //Ö÷»ú·¢Ìø¹ı¶ÁĞòÁĞºÅµÄ²Ù×÷ÃüÁî:0xcc
	WriteDS1820(0x44);                    //Æô¶¯ÎÂ¶È×ª»»
}

void GetAddr(void)  //µÃµ½µ¥Æ¬»úµØÖ·
{
	if(A0) MyAddr+=1;     
	if(A1) MyAddr+=2;
	if(A2) MyAddr+=4;
	if(A3) MyAddr+=8;
	MyAddr1=HEXASC_high(MyAddr);  //ASCµØÖ·
...
/////////////////////////////////////////////
void uart0_ISR(void) interrupt 4  //´®¿ÚÖĞ¶Ï·şÎñ³ÌĞò
{
...
//	uchar imat;
	if(RI0) //ENQ.......EOT,CRC
	{
		...
		if(baud_mode == 0x00)
		{
			if(last==ENQ &&(m==MyAddr ||m=='R' || m=='W'))  Rptr=0;  //¼à²â³õÊ¼¼¸Î»£¬Âú×ãÒªÇó¾ÍÊÇHEX  
			if(Rptr<BufMax) 
			{  
			  Rbuf[Rptr]=m; Rptr++; 
			}
			...
			if(Rptr==3 )
			   {
				  if(Rbuf[0]==MyAddr)   //¼ÇÂ¼ÉÏÎ»»úÊÇ·ñÔÚÑ¯ÎÊ±¾»ú±êÖ¾Î»£¬Èç¹ûÊÇÔòÖÃ1
				  {
			        flag=1; //	SCON0 = SCON0 | 0x20;//ÓÃÀ´ÅĞ¶ÏÊÇµØÖ·ÇëÇó»¹ÊÇÊı¾İÇëÇó
		      	   }
				}
			last=m;
		}
